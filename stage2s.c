...
#define speed  6
#define TEMPS 30
void stage2s(SDL_Surface *ecran,int *continuer,Mix_Music *intro,Mix_Chunk *effet,Mix_Chunk *effet2,Mix_Music *musique,int *mute,SDL_Rect *posvolb,Mix_Chunk *sonspell,Mix_Chunk *dying,Mix_Chunk *hit,int *victory,int *choix)
{
    SDL_Surface *health=NULL,*scoreS=NULL,*Rotation=NULL,*Rotation2=NULL,*Rotation3=NULL,*Rotation4=NULL,*portal=NULL,*ninja=NULL,*shiled=NULL,*shiledr=NULL,*shiledl=NULL,*paragstage1=NULL,*magicien=NULL,*noir=NULL,*gameover=NULL,*attackr=NULL,*attackl=NULL,*hurt=NULL,*rock1=NULL,*rock2=NULL,*spell=NULL,*spellen=NULL,*vie=NULL, *trap1=NULL,*background=NULL,*herod=NULL,*herog=NULL,*heroactu=NULL,*hpup=NULL,*police=NULL,*enemie=NULL,*enemied=NULL,*enemieg=NULL,*enemie1=NULL,*enemied1=NULL,*enemieg1=NULL,*enematack=NULL,*enem1atack=NULL,*score=NULL,*scoreint=NULL,
    ...
    SDL_Rect posback,poshero,posmagicien,posmagicienactu,posspell,posspellen,framed[8],frame2[8],frame3[4],posninja,posninjaa,posshiled,pospolice,posportal,posgo,pose1,posscoreS,pose2,pose3,pose4,pose5,pose6,pose7,pose8,posenemie1,posenemie2,posenemie3,posenemie4,posenemie5,posenemie6,posenemie7,posenemie8,poshp,posrock1,posvie,posscore,poshpup,poshpup1,posminutes,possecondes,posnoir,
    ...
    int y=0,enatack=0,spellenemie=0,nbrspellen=0,heroattack=0;
    int x=0;
    int j=0;
    int stage=0;
    int camera=0;
    ...
    posparagstage1.x=0;
    //position Tous
    charger2(&poshero.x,&posmagicien.x,&posenigme1.x,&poshpup.x,&posback.x,&pospolice.x,&pose1,&pose2,&pose3,&pose4,&pose5,&pose6,&pose7,&posrock1.x,&postrap1.x,&posterre3.x,&posterre1.x,&posterre2.x,&posterre4.x,&posterre5.x,&posterre6.x,&posterre7.x,&posterre8.x,&posterre9.x,&camera,continuer,&stage,&tsec,&tmin,&hp,&cmpsc,&posninja.x,&pose8,&poshpup1.x,&posninjaa.x);
    ...
    posmagicien.y=460;
    posmagicienactu.x=posmagicien.x;//9500
    ...
    posenemie8.x=pose8.x;
    posenemie8.y=pose8.y;
    ...
    //charger tous
    shiledr=IMG_Load ("image/decoration/shield.png");
    shiledl=IMG_Load ("image/decoration/shieldg.png");
    ...
    ninja=IMG_Load ("image/decoration/ninja.png");
...
//	portal=SDL_LoadBMP ("image/decoration/tabscore2.bmp");
    ...
    while ((boucle)&&((*continuer)))
    {
        ...
        while (SDL_PollEvent(&event))
            ...
            case SDL_KEYDOWN:
                ...
                case SDLK_TAB:
                    ...
                    break;
                case SDLK_ESCAPE:
                    Pause2(ecran,&poshero.x,&posmagicien.x,&posenigme1.x,&poshpup.x,&posback.x,&pospolice.x,&posenemie1,&posenemie2,&posenemie3,&posenemie4,&posenemie5,&posenemie6,&posenemie7,&posrock1.x,&postrap1.x,&posterre3.x,&posterre1.x,&posterre2.x,&posterre4.x,&posterre5.x,&posterre6.x,&posterre7.x,&posterre8.x,&posterre9.x,&camera,continuer,tsec,tmin,hp,cmpsc,&posninja.x,&pose8,&poshpup1.x,&posninjaa.x);
                    break;
                case SDLK_m:
                    enigme1(&ecran,victory);
                    break;
                }
                break;
        ...
        if(keystates[SDLK_h])//bouclier
        {
            mouve=0;
            posshiled.y=poshero.y+30;
            if(directionhero==1)
            {
                shiled=shiledr;
                heroactu=attackr;
                posshiled.x=poshero.x+140;
            }
            if(directionhero==2)
            {
                shiled=shiledl;
                heroactu=attackl;
                posshiled.x=poshero.x-60;
            }
            i=28;
        }

        if(keystates[SDLK_d]&&mouve==1&&heroattack==0)//right
        {
            directionhero=1;
            animation(&heroactu,herod,&i);
            if (coll==0)
            {
                scrolling2(&poshero.x,&posmagicien.x,&posenigme1.x,&poshpup.x,&poshpup1.x,&posback.x,&pospolice.x,&pose1.x,&pose2.x,&pose3.x,&pose4.x,&pose5.x,&pose6.x,&pose7.x,&pose8.x,&posrock1.x,&postrap1.x,&posterre3.x,&posterre1.x,&posterre2.x,&posterre4.x,&posterre5.x,&posterre6.x,&posterre7.x,&posterre8.x,&posterre9.x,&posninjaa.x,&camera,&ecran,&heroactu,directionhero);
            }
            else
            {
                poshero.x+=speed;
        ...
        if(keystates[SDLK_q]&&mouve==1&&heroattack==0)//left
        {
            directionhero=2;
        ...
        /*if(keystates[SDLK_j]&&intspell==0&&(jump==0||CAR1==1||CAR2==1||CAR3==1||CAR4==1||CAR5==1||CAR6==1||CAR7==1||CAR8==1||CAR9==1||CAR10==1)&&mouve==1)//attack
        {
                	if(directionhero==1)
                	{
                	        posspell.x=poshero.x+poshero.h/2;
        				...
        				posenemie8.x=pose8.x+compteur1*speed;
        				if(d1==1)
        				{
        				...
        				SDL_BlitSurface(enemie, &framed[e1/4], ecran, &posenemie8);
        ...
        else if(abs(poshero.x-posenemie8.x)<=550&&enatack==0&&posenemie8.y!=2000)
        {
            enemie=enematack;
            enatack=1;
            e1=0;
            posspellen.x=posenemie8.x-posenemie8.h/2;
            posspellen.y=posenemie8.y+30;
        }
        else if(enatack==1)
        {
            if(e1<33)
        ...
        //ninja animation
        positionninja(&ninjacompteur,&dninja);
        animation3(&ninja,ninja,&d2);
        posninja.x=posninjaa.x+ninjacompteur;
        //annimation magicien
        ...
        if ((collision(poshero,posenemie1)==1||collision(poshero,posenemie2)==1||collision(poshero,posenemie3)==1||collision(poshero,posenemie4)==1)||collision(poshero,posenemie5)==1 ||collision(poshero,posenemie6)==1||collision(poshero,posenemie7)==1||collision(poshero,posenemie8)==1)
        {
            if(cmpsc!=0)
                cmpsc-=5;
            if(hp!=0)
        ...
        if(collision(poshero,postrap1)==1&&directionhero==1)
        {
            ...
            for (j=0; j<43; j++)
            {
                ...
                Blit(rock1,ecran,posrock1.x,posrock1.y);
                Blit(hpup,ecran,poshpup.x,poshpup.y);
                Blit(hpup,ecran,poshpup1.x,poshpup1.y);
                Blit(terre1,ecran,posterre1.x,posterre1.y);
                Blit(terre1,ecran,posterre2.x,posterre2.y);
                Blit(terre1,ecran,posterre3.x,posterre3.y);
                Blit(terre2,ecran,posterre4.x,posterre4.y);
                ...
                SDL_BlitSurface(enemie, &framed[j/6], ecran, &posenemie8);
                SDL_BlitSurface(heroactu, &frame2[j/10], ecran, &poshero);
                SDL_BlitSurface(ninja, &frame3[j/12], ecran, &posninja);
                SDL_BlitSurface(vie, NULL, ecran, &posvie);
                SDL_BlitSurface(imgtime, NULL, ecran, &postime);
                SDL_BlitSurface(health, NULL, ecran, &poshp);
                SDL_BlitSurface(score, NULL, ecran, &posscore);
        ...
        if(collision(poshero,postrap1)==1&&directionhero==2)
        {
            ...
            for (j=0; j<43; j++)
            {
                ...
                SDL_BlitSurface(enemie, &framed[j/6], ecran, &posenemie8);
                SDL_BlitSurface(heroactu, &frame2[j/10], ecran, &poshero);
                SDL_BlitSurface(ninja, &frame3[d2/4], ecran, &posninja);
                SDL_BlitSurface(vie, NULL, ecran, &posvie);
                SDL_BlitSurface(imgtime, NULL, ecran, &postime);
                SDL_BlitSurface(health, NULL, ecran, &poshp);
                SDL_BlitSurface(score, NULL, ecran, &posscore);
        ...
        //collision avec ninja
        if(collision(poshero,posninja)==1&&directionhero==1)
        {
            mouvemmentsouri=0;
            coll=1;
            poshero.x-=100;
        ...
        if(collision(poshero,posninja)==1&&directionhero==2)
        {
            mouvemmentsouri=0;
            coll=1;
            poshero.x+=100;
        ...
        if(collision(posspellen,posshiled)==1)
        {
            posspellen.y=2000;
            enatack=0;
            nbrspellen=0;
        ...
        if(collision(posspell,posenemie8)==1)
        {
            spellenemie=0;
            posspellen.y=2000;
            cmpsc+=10;
        ...
        if(collision(poshero,poshpup1)==1)
        {
            if(hp!=4)
            {
                hp++;
        ...
        SDL_BlitSurface(surfacesecondes, NULL, ecran, &possecondes);
        SDL_BlitSurface(surfaceminutes, NULL, ecran, &posminutes);
        Blit(shiled,ecran,posshiled.x,posshiled.y);
        SDL_Flip(ecran);
    }
